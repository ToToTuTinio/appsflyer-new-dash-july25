...
def run_reports_sequentially(app):
    """
...
def run_scheduled_reports(app):
    ...
    schedule.every(6).hours.do(run_reports_sequentially, app)
    
    # Run immediately on startup
    run_reports_sequentially(app)
...
def start_report_service(app):
    """
    Start the report service in a separate thread
    """
    report_thread = threading.Thread(target=run_scheduled_reports, args=(app,), daemon=True)
    report_thread.start()
